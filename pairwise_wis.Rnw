...
\item Compute the pairwise relative WIS skill $\theta_{ij}^{\text{perm}}$ from the permuted scores.
\end{enumerate}
We then generate a large number of samples from the reference distribution. To compute a \textbf{two-sided} $p$-value we compute the fraction of samples where $\max(\theta_{ij}^{\text{perm}}, \theta_{ji}^{\text{perm}})$ exceeds the observed $\max(\theta_{ij}, \theta_{ji})$. (Previously I had described a one-sided test here, but the implementation was always two-sided).



\section{Longer justification of the approach}

\textit{I tried to update this to the newest notation and terminology}
...
\begin{itemize}
\item The \textit{relative WIS} with respect to the baseline model:
\begin{equation}
...
\item The relative WIS skill with respect to the set of models $\mathcal{M}$:
\begin{equation}
...
To get a better intuition of what $\theta^{*}_{i}$ actually measures consider the case where there are only two forecasters. Assume that the baseline forecaster is available for all targets and denote the subset of forecast targets covered by forecaster $i$ by $\mathcal{A}_{i}$. Then we have

\begin{align}
\theta^*_1 = & \left(\frac{\theta_{1, 1} \theta_{1, 2}}{\theta_{B, 1} \theta_{B, 2}}\right)^{1/2}\\
= & \dots \\
= & \bigg(
\frac{\text{mean WIS of model 1 on } \mathcal{A}_1}{\text{mean WIS of baseline } B  \text{ on } \mathcal{A}_1} \\
 & \ \ \times
\frac{\text{mean WIS of model 1 on } \mathcal{A}_{1, 2} \times \frac{\text{mean WIS of model 2 on } \mathcal{A}_{2}}{\text{mean WIS of model 2 on } \mathcal{A}_{1, 2}}}{\text{mean WIS of baseline } B  \text{ on } \mathcal{A}_2}
\bigg)^{1/2}
\end{align}

One can thus interpret $\theta^*_1$ as the geometric mean of two quantities:
\begin{enumerate}
\item The performance of model 1 relative to the baseline on the set of forecast targets $\mathcal{A}_1$ covered by model 1.
\item An estimate of the performance model 1 would have achieved relative to the baseline on the set of forecasts $\mathcal{A}_2$ covered by model 2. The mean WIS of model 1 on $\mathcal{A}_2$ often cannot be evaluated. As an estimate, we multiply the mean WIS achieved on $\mathcal{A}_{1, 2}$ (the subset of $\mathcal{A}_2$ also covered by model 1) with the ratio of the mean WIS model 2 achieved on the full $\mathcal{A}_2$ and the subset $\mathcal{A}_{1, 2}$. We thus assume that the``relative difficulty'' of forecasting $\mathcal{A}_{1, 2}$ and $\mathcal{A}_2 \backslash \mathcal{A}_{1, 2}$ would have been the same for model 1 as it was for model 2. Note that this reduces to the observed mean WIS of model 1 on $\mathcal{2}$ if model 1 covered all of $\mathcal{A}_2$.
\end{enumerate}

This also generalizes to a larger number of models $M$ included in the comparison. The summary score $\theta^*_i$ can be interpreted as
\begin{equation}
\theta^*_i = \left(\prod_{m = 1}^M \frac{\text{estimated mean WIS model } i \text{ on } \mathcal{A}_{m}}{\text{mean WIS of baseline model } B \text{ on } \mathcal{A}_{m}}\right)^{1/M}.
\label{eq:intuition}
\end{equation}
To estimate the mean WIS of model $i$ on $\mathcal{A}_{m}$, we use the results of model $m$ as a reference to determine the relative difficulty of the forecast targets in $\mathcal{A}_{i, m}$ and $\mathcal{A}_m$. If model $i$ covered all $\mathcal{A}_m$ anyway, no correction is performed and the results obtained by model $m$ have no impact on the summary score $\theta^*_i$ of model $i$. However, equation \ref{eq:intuition} implies that we give larger weight to forecast targets covered by more models, as these enter in term \eqref{eq:intuition} more often. This means that models covering only part of the targets are allowed to ``shift the focus'' of the evaluation towards the targets they chose to cover. In a sense, each model is given an equal weight in determining on which set of targets all models are evaluated.
...
<<tab_skill, echo=FALSE, results="asis", cache=TRUE>>=
